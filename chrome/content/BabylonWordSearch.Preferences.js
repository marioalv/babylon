...
BabylonWordSearch.Preferences = {
  ...
  init: function()
  {
    ...
    this.menuList = document.getElementById('babylon-word-search-languages-list');
    ...
    this.keyboardShortcutMessageTitle =
      document.getElementById('babylon-word-search-keyboard-shortcut-title-message');
    ...
    this.keyboardShortcutMessage.value =
      this.stringBundle.getString("babylonWordSearch.messages.shortcutInfoMessage");

    BabylonWordSearch.Utils.createLanguagesMenulist(this.menuList, this.stringBundle);
  ...
  handleShortcutSet: function(event)
  {
    ...
    window.addEventListener("keyup",
                            BabylonWordSearch.Preferences.handleKeyupEvent,
                            true);
  },
  ...
  handleKeyupEvent: function(event)
  {
    ...
    switch(BabylonWordSearch.Utils.validateShortcutCombination(event))
    {
      ...
      case BABYLON_WORD_SEARCH_ERROR_CODES_REQUIRED_MODIFIER_NOT_PRESENT:
        ...
        BabylonWordSearch.Preferences.setShortcutValidationMessage(
          ...
          BabylonWordSearch.Preferences.stringBundle.getString('babylonWordSearch.messages.requiredModifierNotPresent'));
      ...
      case BABYLON_WORD_SEARCH_ERROR_CODES_KEYSTROKE_NOT_ALLOWED:
        ...
        BabylonWordSearch.Preferences.setShortcutValidationMessage(
          ...
          BabylonWordSearch.Preferences.stringBundle.getString('babylonWordSearch.messages.keystrokeNotAllowed'));
        return;
    }
    ...
    BabylonWordSearch.Preferences.setShortcutValidationMessage(
      ...
      BabylonWordSearch.Preferences.stringBundle.getString('babylonWordSearch.messages.everythingOK'));
    ...
    BabylonWordSearch.Preferences.keyboardShortcutButton.label =
      BabylonWordSearch.Preferences.stringBundle.getString('babylonWordSearch.setShortcut');

    BabylonWordSearch.Preferences.keyboardShortcutMessageTitle.value = BABYLON_WORD_SEARCH_EMPTY_STRING;

    var newShortcutString = BabylonWordSearch.Utils.processValidShortcutInput(event);
    BabylonWordSearch.Preferences.keyboardShortcutTextbox.value = newShortcutString;
    BabylonWordSearch.Preferences.prefManager.setCharPref('babylonWordSearch.keyboardShortcut',
                                                          newShortcutString);
  ...
  /**
   * result: tells the user if the shortcut is valid or not, by setting a message in a textbox below the shortcut set textbox
   */
  setShortcutValidationMessage: function(errorType, message)
  {
    if (errorType != BABYLON_WORD_SEARCH_ERROR_CODES_EVERYTHING_OK) {
      this.keyboardShortcutMessage.setAttribute('style','color:red;font-weight:bold;');
    }
    else {
      this.keyboardShortcutMessage.setAttribute('style','color:black;font-weight:bold;');
    }
...
window.addEventListener("load",
                        function () { BabylonWordSearch.Preferences.init() },
                        false);
